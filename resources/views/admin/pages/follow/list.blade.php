                    ...
                    <div class="col-xs-12">
                        <div class="box">
                            <div class="box-header">
                                <a href="{{route('follow.create')}}" class="btn btn-success">Thêm theo dõi </a>
                            </div>
                            <div class="box-header">
                                ...
                                <table id="example1" class="table table-bordered table-hover">
                                    ...
                                    @foreach($follow as $value)
                                        <tr class="odd gradeX" align="center">
                                            <td>{{$value->name}}</td>
                                            <td><img width="100px" src="assets/img_follow/{{$value->logo}}"></td>
                                            <td>{{$value->link}}</td>
                                            <td>
                                                @if($value->status==1)
                                                    Hiển thị
                                            ...
                                            <td>
                                                ...
                                                <div id="button{{$value->id}}">
                                                    ...
                                                    <a class="btn btn-success"
                                                       href="{{url('admincp/follow/show/'.$value->id)}}"> Xem</a>


                                                    @if($value->status==1)
                                                        <a class="btn btn-info"
                                                           href="{{ url('admincp/follow/setactive/'.$value->id.'/0') }}"
                                                           onclick="return confirm('Hành động sẽ ẩn Sản Phẩm này! bạn có muốn tiếp tục?')">Ẩn</a>
                                                    @else
                                                        <a class="btn btn-warning"
                                                           href="{{ url('admincp/follow/setactive/'.$value->id.'/1') }}"
                                                           onclick="return confirm('Hành động sẽ hiển thị Sản Phẩm mục này! bạn có muốn tiếp tục?')">Hiển
                                                            thị</a>

                                                    @endif
                                                    <a class="btn btn-danger"
                                                       href="{{ url('admincp/follow/destroy/'.$value->id) }}"
                                                       onclick="return confirm('Hành động sẽ xóa theo dõi  này! bạn có muốn tiếp tục?')">Xóa</a>
                                                </div>
